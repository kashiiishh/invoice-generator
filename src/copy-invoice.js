...
const InvoiceGenerator = () => {
    ...
    const generateInvoice = () => {
        if (invoiceNumber !== null && invoiceNumber !== undefined) {
            ...
            alert(`Invoice #${invoiceNumber} generated!`); // Show the current invoice number
    ...
    //   // sgstAndCgst = (sgstAndCgst !== undefined && sgstAndCgst !== null) ? sgstAndCgst : 0;
    //   // discount = (discount !== undefined && discount !== null) ? discount : 0;
    //   // totalPayable = (totalPayable !== undefined && totalPayable !== null) ? totalPayable : 0;



    //   // total = typeof total === 'number' && !isNaN(total) ? total : 0;
    //   // sgstAndCgst = typeof sgstAndCgst === 'number' && !isNaN(sgstAndCgst) ? sgstAndCgst : 0;
    //   // discount = typeof discount === 'number' && !isNaN(discount) ? discount : 0;
    //   // totalPayable = typeof totalPayable === 'number' && !isNaN(totalPayable) ? totalPayable : 0;
    ...
    //   // Summary Details
    //   const spacing = 6;
    //   doc.text(`Total (Inclusive of SGST & CGST): Rs.${roundToTwoDecimals(total)}`, 10, lastY);
    //   doc.text(`SGST (2.5%): Rs.${roundToTwoDecimals(sgstAndCgst)}`, 10, lastY + spacing);
    //   doc.text(`CGST (2.5%): Rs.${roundToTwoDecimals(sgstAndCgst)}`, 10, lastY + spacing * 2);

    //   if (discount > 0) {
    //     doc.text(`Discount: Rs.${roundToTwoDecimals(discount)}`, 10, lastY + spacing * 3);
    //   }
    ...
    //   doc.setFillColor(240, 240, 240);
    //   doc.rect(10, totalPayableY - 5, 190, 10, 'FD');
    //   doc.text(`Total Payable: Rs.${roundToTwoDecimals(totalPayable)}`, 105, totalPayableY, null, null, 'center');
    ...
    //     // Emit an error if critical values are missing
    //     if (
    //         total === null ||
    //         total === undefined ||
    //         sgstAndCgst === null ||
    //         sgstAndCgst === undefined ||
    //         discount === null ||
    //         discount === undefined ||
    //         totalPayable === null ||
    //         totalPayable === undefined
    //     ) {
    //         console.error("Error: One or more required values are missing or invalid.");
    //         alert("Please ensure all invoice values are calculated correctly before generating the PDF.");
    //         return;
    //     }
    ...
    //     // Check position after the table
    //     const lastY = doc.lastAutoTable.finalY + 10;
